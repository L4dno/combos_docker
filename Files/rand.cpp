...
double hyperx3(std::array<double, 3> x, std::array<double, 3> s)
{ /* 'hyperx' returns a psuedo-random variate from three-   */
  ...
  boost::variate_generator<boost::mt19937 &, boost::random::hyperexponential_distribution<>> gamma_gen(rng, distr);
...
{ /* 'normal' returns a psuedo-random variate from a normal dis-  */
  ...
  else
  {
    do
    {
      v1 = 2.0 * uniform_pos() - 1.0;
      v2 = 2.0 * uniform_pos() - 1.0;
      w = v1 * v1 + v2 * v2;
    } while (w >= 1.0);
    w = sqrt((-2.0 * log(w)) / w);
    z1 = v1 * w;
...
double ran_distri(char num, std::vector<double> p, std::vector<double> u)
{
  if (num == 8)
  {
    std::array<double, 3> arr_p = {p[0], p[1], p[2]};
    std::array<double, 3> arr_u = {u[0], u[1], u[2]};
    return hyperx3(arr_p, arr_u);
  }
  double a = p[0], b = u[0];
  switch (num)
  {
